...
import { Link, styled, Alert } from '@mui/material';
...
const AddExpense = (props) => {
    ...
    const recognizeText = async () => {
        ...
        //         setInput({...input,['title']:data.ParsedResults[0].ParsedText});
        //     }
        // } catch (error) {
    ...
    const [input, setInput] = useState({
      ...
      const add = () => {
        axios.post("http://localhost:8080/addexpense?isOCR=" + isOCR,input,{headers:headers}).then(
          (response)=>{
            ...
            >
                <React.Fragment> 
                    ...
                    >
                        ...
                        {isOCR && <DialogContent sx={{fontSize:'small'}}>
                    ...
                    <DialogActions>
                    <Grid item xs={3} sm={4} md={4} className='addbutton-row'>
                      <Link onClick={() => { setIsOCR(false) }} style={{ color: 'black', opacity: '0.8', cursor: 'pointer', width: '60%' }}>Skip to manual expense addition</Link>
                      {alert.includes('success') &&
                      <Alert sx={{ padding: '0px 16px', width: '40%', margin: '0px 10px' }} variant="outlined" severity="success">
                        {alert}
                      </Alert>}
                      {alert && !alert.includes('success') &&
                      <Alert sx={{ padding: '0px 16px', width: '40%',  margin: '0px 10px' }} variant="outlined" severity="error">
                        {alert}
                      </Alert>}
                      <Button id='ocrAddBtn' disabled={ocrText === ""} autoFocus variant="contained" size='medium' type='submit' onClick={addExpense}>
                          Add
                      </Button>
                    </Grid>
                    </DialogActions>
                ...
                <DialogActions>
                  <Grid item xs={3} sm={4} md={4} spacing={2} className='addbutton-row'>
                    <Link onClick={() => { setIsOCR(true) }} style={{ color: 'black', opacity: '0.8', cursor: 'pointer', width: '60%' }}>Go to OCR</Link>
                    {alert.includes('success') &&
                    <Alert sx={{ padding: '0px 16px', width: '40%', margin: '0px 10px' }} variant="outlined" severity="success">
                      {alert}
                    </Alert>}
